...
import { Search, MapPin, Calendar, Users, Star, Plane, Mountain, Ship, Building2, Award, Shield, Headphones, TrendingUp, ShoppingCart, Loader2 } from 'lucide-react';
...
const staticFeaturedTours: Tour[] = [
  {
    ...
    rating: 4.9,
    reviews: 1234,
    description: "Khám phá kỳ quan thiên nhiên thế giới với hàng ngàn đảo đảo đá vôi kỳ vĩ"
  },
  {
    id: '2',
    ...
    <div className="min-h-screen bg-white">
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="px-4 py-2 text-gray-700 hover:text-blue-600 transition-colors font-semibold"
                  >
                    {user.name}
                  </button>
                  <button
                    onClick={logout}
                    className="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors font-semibold cursor-pointer"
                  >
                    Đăng xuất
                  </button>
                </div>
                ...
                <>
                  <button
                    ...
                    className="px-4 py-2 text-gray-700 hover:text-blue-600 transition-colors font-semibold cursor-pointer"
                  >
                    Đăng nhập
                  </button>
                  <button
      ...
      <section className="relative h-[600px] bg-gradient-to-r from-blue-900 to-blue-700">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col justify-center items-center text-center">
          <h1 className="text-5xl md:text-6xl text-white mb-6 max-w-4xl font-bold">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="flex items-center gap-3 p-3 border rounded-lg">
                ...
                <div className="flex-1 text-left">
                  <label className="text-xs text-gray-500 block font-medium">Điểm đến</label>
                  <input
                    type="text"
              ...
              <div className="flex items-center gap-3 p-3 border rounded-lg">
                ...
                <div className="flex-1 text-left">
                  <label className="text-xs text-gray-500 block font-medium">Ngày đi</label>
                  <input
                    type="date"
                    value={searchQuery.date}
                    onChange={(e) => setSearchQuery(prev => ({ ...prev, date: e.target.value }))}
                    className="w-full outline-none text-gray-900 font-semibold bg-transparent"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-3 border rounded-lg">
                ...
                <div className="flex-1 text-left">
                  <label className="text-xs text-gray-500 block font-medium">Số người</label>
                  <select
                    value={searchQuery.guests}
                    onChange={(e) => setSearchQuery(prev => ({ ...prev, guests: e.target.value }))}
                    className="w-full outline-none text-gray-900 bg-transparent font-semibold"
                  >
              ...
              <button
                ...
                className="bg-blue-600 text-white rounded-lg px-6 py-3 hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 font-bold cursor-pointer"
              >
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-center mb-12 text-gray-900">Danh mục tour</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {categories.map((category, index) => (
              <div
                key={index}
                className="bg-white p-8 rounded-xl text-center hover:shadow-lg transition-shadow cursor-pointer group border border-gray-100"
              >
                <div className="inline-flex items-center justify-center text-blue-600 mb-4 group-hover:scale-110 transition-transform">
                  {category.icon}
                </div>
                <h3 className="text-lg text-gray-900 mb-2 font-semibold">{category.name}</h3>
                <p className="text-sm text-gray-500 font-semibold">{category.count} tours</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="tours" className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl text-gray-900 mb-4 font-bold">Tour nổi bật</h2>
            <p className="text-lg text-gray-600 font-medium">Những tour du lịch được yêu thích nhất</p>
          </div>

          {loading ? (
            <div className="flex justify-center items-center py-12">
              <Loader2 className="size-8 animate-spin text-blue-600" />
              <span className="ml-3 text-gray-600 font-semibold">Đang tải danh sách tour...</span>
            </div>
          ) : (
      ...
      <section className="py-16 bg-blue-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-4xl font-bold text-center mb-4 text-gray-900">Tại sao chọn chúng tôi?</h2>
          <p className="text-center text-gray-600 mb-12 font-medium">Những lý do khách hàng tin tưởng TravelHub</p>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {benefits.map((benefit, index) => (
              <div key={index} className="bg-white p-8 rounded-xl text-center hover:shadow-lg transition-shadow border border-gray-100/50">
                <div className="inline-flex items-center justify-center text-blue-600 mb-4">
                  {benefit.icon}
                </div>
                <h3 className="text-xl text-gray-900 mb-2 font-bold">{benefit.title}</h3>
                <p className="text-gray-600 text-sm font-semibold">{benefit.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-4xl font-bold text-center mb-12 text-gray-900">Khách hàng nói gì về chúng tôi</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={index} className="bg-white p-6 rounded-xl border hover:shadow-lg transition-shadow text-left">
                <div className="flex gap-1 mb-4">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star key={i} className="size-5 fill-yellow-400 text-yellow-400" />
                  ))}
                </div>
                <p className="text-gray-700 mb-4 italic text-sm font-semibold">"{testimonial.comment}"</p>
                <div>
                  <p className="text-gray-900 font-bold">{testimonial.name}</p>
                  <p className="text-xs text-gray-500 font-semibold">{testimonial.location}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-16 bg-gradient-to-r from-blue-600 to-blue-800">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl text-white mb-4 font-bold">Đăng ký nhận ưu đãi</h2>
          <p className="text-xl text-white/90 mb-8 font-medium">Nhận thông tin tour mới và ưu đãi đặc biệt qua email</p>

          <div className="flex flex-col sm:flex-row gap-4 max-w-xl mx-auto">
            <input
              type="email"
              placeholder="Nhập email của bạn"
              className="flex-1 px-6 py-3 rounded-lg outline-none text-gray-900 font-semibold"
            />
            <button className="px-8 py-3 bg-white text-blue-600 rounded-lg hover:bg-gray-100 transition-colors font-bold cursor-pointer">
              Đăng ký ngay
            </button>
          </div>
        </div>
      ...
      <footer className="bg-gray-900 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8 text-left">
            ...
            <div>
              <h3 className="text-lg mb-4 font-bold">Công ty</h3>
            ...
            <div>
              <h3 className="text-lg mb-4 font-bold">Liên hệ</h3>
              <ul className="space-y-2 text-gray-400 text-sm font-medium">
          ...
          <div className="pt-8 border-t border-gray-800 text-center text-gray-400 text-sm font-medium">
