...
type AdminTab = 'overview' | 'orders' | 'payments';
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [toursResponse, bookingsResponse, hotelBookingsResponse, roomsResponse, summaryResponse, transactionsResponse] = await Promise.all([
        fetch(apiUrl('/api/tours/admin'), { headers: authHeaders }),
  ...
  const tabTitles: Record<AdminTab, { title: string; subtitle: string }> = {
    overview: { title: 'Tổng quan kế toán', subtitle: 'Báo cáo doanh thu & dòng tiền' },
    orders: { title: 'Quản lý đơn đặt', subtitle: 'Phê duyệt & Hủy đơn' },
    payments: { title: 'Đối soát giao dịch', subtitle: 'Lịch sử thanh toán & Trạng thái' },
  } as any;

  return (
    <div className={`min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white flex ${theme === 'dark' ? 'dark' : ''}`}>
      <aside className="hidden xl:flex w-70 flex-col border-r border-slate-200/70 dark:border-slate-800 bg-slate-100/80 dark:bg-slate-900/80 backdrop-blur-sm h-screen sticky top-0">
        <div className="p-8">
        ...
        <div className="mt-auto p-6 border-t border-slate-200/70 dark:border-slate-800">
          <div className="flex items-center gap-3">
            {user.avatar ? (
              ...
              <div className="w-12 h-12 rounded-full bg-slate-300 dark:bg-slate-700 flex items-center justify-center text-white text-sm font-bold">
                {getInitials(user.name)}
              </div>
            )}
            <div className="text-left">
              <p className="font-bold">{user.name}</p>
              <p className="text-sm text-slate-500 dark:text-slate-400">Nhân viên Kế toán</p>
            </div>
          </div>
